...
fn the_roster_lists_each_account_with_its_standing() {
    ...
    assert_eq!(cx.host().asked::<Live<Identity>>().len(), 1);
}
...
fn a_live_bump_re_reads_and_a_snapshot_restores_the_screen() {
    let mut cx = TestAppContext::new();
    let feed = cx.host().stream::<Live<Identity>>();
    respond(&mut cx);
    cx.open::<Members>();
    cx.run_until_parked();
    cx.host()
    ...
    restored.host().stream::<Live<Identity>>();
    ...
    assert_eq!(restored.host().asked::<Live<Identity>>().len(), 1);
}
