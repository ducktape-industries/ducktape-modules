...
pub fn save_bounds<S: Writes>(sandbox: &mut S, bounds: &Bounds) {
    sandbox.set(BOUNDS.to_vec(), abi::encode(bounds));
}

pub fn load_bounds<S: Reads>(sandbox: &S) -> Result<Bounds, Refusal> {
...
pub fn save_repo<S: Writes>(sandbox: &mut S, name: &str, repo: &Repo) {
...
pub fn load_repo<S: Reads>(sandbox: &S, name: &str) -> Result<Repo, Refusal> {
...
pub fn repo_exists<S: Reads>(sandbox: &S, name: &str) -> bool {
    sandbox.get(repo_key(name)).is_some()
}

pub fn is_writer<S: Reads>(sandbox: &S, name: &str, key: &[u8]) -> bool {
    sandbox.get(writer_key(name, key)).is_some()
}

pub fn load_refs<S: Reads>(
    sandbox: &S,
    name: &str,
    hash: Hash,
...
pub fn set_ref<S: Writes>(sandbox: &mut S, name: &str, reference: &[u8], target: &Oid) {
    sandbox.set(ref_key(name, reference), target.as_bytes().to_vec());
}

pub fn delete_ref<S: Writes>(sandbox: &mut S, name: &str, reference: &[u8]) {
...
pub fn load_ref<S: Reads>(
    ...
    sandbox
        ...
        .map(|b| Oid::from_bytes(hash, &b).map_err(|e| storage(e.to_string())))
        .transpose()
}

/// Resolving an op's endpoint reads consensus refs only, never objects.
pub fn resolve<S: Reads>(
    sandbox: &S,
    name: &str,
