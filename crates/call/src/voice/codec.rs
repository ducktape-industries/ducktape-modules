...
pub enum CodecError {
    ...
    /// what a lane's `bad_packet` counter carries.
    #[error("bad packet: {0}")]
...
impl VoiceDecoder {
    ...
    /// never the whole session. (On wasm32 the guest panics abort, so the
    /// guard is a no-op there and the TOC check is the whole defense — see
    /// `tests::no_short_packet_panics`, which runs under wasm for that reason.)
...
pub mod tests {
    ...
    pub fn tone(tick: usize) -> [i16; FRAME_SAMPLES] {
        let mut pcm = [0i16; FRAME_SAMPLES];
    ...
    #[cfg_attr(test, test)]
    pub fn round_trip_preserves_energy() {
    ...
    #[cfg_attr(test, test)]
    pub fn a_sixty_millisecond_toc_is_refused_and_the_lane_keeps_decoding() {
        let mut dec = VoiceDecoder::new().unwrap();
        assert!(matches!(
