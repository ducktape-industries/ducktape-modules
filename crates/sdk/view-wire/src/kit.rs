...
use crate as wire;
use crate::{AlignX, AlignY, Axis, ButtonContent, ButtonPreset, Length, Node, Role};
...
pub fn width(node: Node, width: Length) -> Node {
    sized(node, Some(width), None)
}
pub fn height(node: Node, height: Length) -> Node {
    sized(node, None, Some(height))
}
pub fn fill_width(node: Node) -> Node {
    sized(node, Some(Length::Fill), None)
}
pub fn fill(node: Node) -> Node {
    sized(node, Some(Length::Fill), Some(Length::Fill))
}
pub fn padded_all(node: Node, all: f32) -> Node {
    padded(node, wire::Edges::all(all))
}
...
pub fn aligned_x(mut node: Node, align: AlignX) -> Node {
    match &mut node {
        ...
        Node::Linear { .. } => return aligned(node, align),
        _ => {}
    }
    node
}
...
pub fn text_field(
    ...
    let mut node = input(key, label, value, on_input, on_submit);
...
pub fn section_row(key: &str, name: &str, control: Option<Node>) -> Node {
    let mut children = vec![fill_width(nowrap(label(format!("{key}/label"), name)))];
    children.extend(control);
    padded(
        height(
            fill_width(spaced(centered_row(key, children), spacing::XXS as f32)),
            Length::Fixed(height::CONTROL as f32),
        ),
        wire::Edges {
            top: 0.,
            right: spacing::XXS as f32,
            bottom: 0.,
            left: spacing::SM as f32,
        },
    )
...
pub fn sidebar_row(mut button: Node, name: &str) -> Node {
    ...
    } = &mut button
    {
        ...
        *height = Some(Length::Fixed(height::CONTROL as f32));
        *padding = Some(wire::Edges {
            top: 0.,
            right: spacing::SM as f32,
            bottom: 0.,
...
pub fn unread_dot(key: String) -> Node {
    width(
        rounded(
            background(
                container(key, space(Some(Length::Fixed(8.)), Some(Length::Fixed(8.)))),
                palette().accent,
            ),
            radius::PILL as f32,
        ),
...
/// A modal layer over `under`: `over` sits at the alignment inside the
/// padding, the backdrop tints what is beneath, a press outside dismisses.
...
/// A 10px grab strip between two panes with the hairline down its middle;
/// `on_drag` takes the `(f64, f64)` delta.
pub fn resize_handle(key: &str, on_drag: u32) -> Node {
    let strip = aligned_x(
        height(
            width(
                container(
                    format!("{key}/strip"),
                    vertical_divider(format!("{key}/rule")),
                ),
                Length::Fixed(10.),
            ),
            Length::Fill,
...
pub fn pane_header(key: &str, children: impl IntoIterator<Item = Node>) -> Node {
    padded(
        height(fill_width(centered_row(key, children)), Length::Fixed(40.)),
        wire::Edges {
            top: 0.,
            right: spacing::SM as f32,
            bottom: 0.,
...
pub fn gives_way(key: &str, title: Node) -> Node {
    clipped(width(container(key, nowrap(title)), Length::Shrink))
}

/// Where a popup of `size` sits for a press inside `viewport`: its top-left
/// at the pointer, flipped left or up at an edge, never past the corner.
pub fn popup_origin(press: (f32, f32), size: (f32, f32), viewport: (f32, f32)) -> (f32, f32) {
    const GUTTER: f32 = 8.;
