...
fn room_view(chat: &mut Chat, room_id: &str, cx: &mut Cx<Chat>) -> Node {
    ...
    if let Some(room) = chat.room.as_mut()
        && let Loaded::Ready(rows) = &room.messages
    {
        room.pending
            .retain(|pending| !rows.iter().any(|row| row.message_id == pending.message_id));
    }
    let rows = chat.room.as_ref().map(|room| &room.messages);
    let timeline = match rows {
        Some(Loaded::Ready(rows)) => {
            let pending = chat.room.as_ref().map(|room| &room.pending);
            let rows: Vec<MsgRow> = rows
                .iter()
        ...
        Some(Loaded::Ready(replies)) => {
            ...
            if let Some(room) = &chat.room {
                rows.extend(
                    room.pending
                        .iter()
                        .filter(|row| row.thread == Some(root))
                        .cloned(),
                );
            }
...
fn message_view(
    ...
    if !message.deleted && message.seq > 0 {
