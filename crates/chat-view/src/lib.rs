...
struct Room {
    ...
    /// Sends the module accepted that the index has not shown yet; drawn after
    /// the fetched rows and dropped once a fetched row carries the same id.
    #[serde(skip)]
    pending: Vec<MsgRow>,
}
...
fn pending_row(op: &ChatMsg) -> Option<MsgRow> {
