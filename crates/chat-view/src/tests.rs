...
fn boot_reads_the_roster_and_the_rooms_and_a_room_opens_on_press() {
    ...
    // an accepted send shows as a pending row until the index serves it
    let pending = MsgRow {
        seq: 0,
        message_id: "p1".into(),
        author: "acct:8".into(),
    ...
    let frame = driver.tick(type_into(&frame, "dm-peer", "acct:8"));
    assert!(has_text(&frame, "on its way") && has_text(&frame, "sending…"));
    {
        let mut chat = driver.app.state_mut();
        let room = chat.room.as_mut().unwrap();
        let Loaded::Ready(rows) = &mut room.messages else {
            panic!("rows loaded")
        };
        rows.push(MsgRow { seq: 2, ..pending });
    }
    let frame = driver.tick(type_into(&frame, "dm-peer", "acct:9"));
    assert!(has_text(&frame, "on its way") && !has_text(&frame, "sending…"));
    assert!(driver.app.state().room.as_ref().unwrap().pending.is_empty());
