...
const COLLABORATION_WASM: &[u8] = include_bytes!("../component.wasm");
    ...
    async fn query(&self, req: &[u8]) -> Result<Vec<u8>, Error> {
        let reply = match chat::decode_query(req).map_err(|sentence| Error::Module {
            ...
            chat::ChatQuery::Message { message_id } => {
                ...
                producer::ChatReply::Message(seq.map(|seq| {
                    let mut message = producer::message(
                        "c1",
                        &message_id,
                        seq,
                        party(SERVICE),
                        Origin::External(vec![SERVICE; 32]),
                        "please review",
                        None,
                    );
                    message.head.created_at = seq;
                    message
                }))
            }
            other => {
...
fn same_ops_same_roots_and_the_same_authenticated_replies() {
    deterministic::Runner::default().start(|context| async move {
        ...
        for (index, (signer, read)) in reads().into_iter().enumerate() {
            ...
            let n = native
                .authenticated_read(signer, height, read.clone())
                .await;
